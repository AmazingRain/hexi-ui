...
const SubMenu: React.FC<SubMenuProps> = (props) => {
  ...
  const renderChildren = () => {
    ...
    const childrenComponent = React.Children.map(children, (child, i) => {
      ...
      if (displayName === 'MenuItem') {
        ...
        console.warn(
          'Warning: The child component of SubMenu must be MenuItem',
        );
      }
    });

    return <ul className={classNames}>{childrenComponent}</ul>;
  };
  ...
  const clickEvents =
    mode === 'vertical'
      ...
      : {};
  return (
    <li key={index} className={classNames} {...hoveEvents}>
